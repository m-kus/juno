...
// TestMarshalJSON_ValueInInterface reproduces a bug where felt types serialized
...
func TestMarshalJSON_ValueInInterface(t *testing.T) {
	...
	tests := []struct {
		name   string
		input  any
		broken bool // true = currently broken (limbs leak)
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Contains(t, s, "0xdeadbeef",
				"expected hex string, got limbs — pointer-receiver MarshalJSON not called on non-addressable value")
